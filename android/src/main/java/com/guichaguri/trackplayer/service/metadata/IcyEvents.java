...
public class IcyEvents implements IcyHttpDataSource.IcyMetadataListener, IcyHttpDataSource.IcyHeadersListener {
  ...
  public IcyEvents(MusicManager manager, String userAgent) {
    this.manager = manager;

    this.ds = new IcyHttpDataSourceFactory.Builder(OkHttpClientProvider.getOkHttpClient()).setUserAgent(userAgent)
      .setIcyMetadataChangeListener(this).setIcyHeadersListener(this).build();
  }
  ...
  public void onIcyMetaData(IcyHttpDataSource.IcyMetadata metadata) {
    ...
    manager.onMetadataUpdate(bundle);
  }
